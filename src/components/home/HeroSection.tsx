...
export function HeroSection() {
  ...
  const stats = [
    ...
    { icon: null, topLabel: 'VITRINE', label: "Observatório da Juventude\nesse espaço é seu ", cta: "Conecte-se aqui", link: '/cadastro' },
            ...
            {stats.map((stat) => {
              const content = (
                <div className={`flex flex-col items-center p-6 rounded-2xl backdrop-blur-md border card-hover transition-all duration-300 bg-primary/20 border-primary/40 shadow-[0_0_25px_rgba(14,165,233,0.15)] hover:shadow-[0_0_35px_rgba(14,165,233,0.25)] hover:scale-105 ${stat.link ? 'cursor-pointer' : ''}`}>
                  {stat.topLabel && (
                    <span className="text-3xl tracking-wide text-[#38bdf8] drop-shadow-[0_0_12px_rgba(56,189,248,0.5)] mb-1 font-extrabold" style={{ fontFamily: "'Nunito', 'Quicksand', sans-serif" }}>
                      {stat.topLabel}
                    </span>
                  )}
